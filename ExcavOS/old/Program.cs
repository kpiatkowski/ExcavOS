...
namespace IngameScript
{
    partial class Program : MyGridProgram
    {
        ...
        private void CollectCockpits()
        {
            ...
            {
                if (!cockpits.ContainsKey(cockpit.EntityId) && (BlockHelper.IsCockpitIndustrial(cockpit) || BlockHelper.IsStandardCockpit(cockpit)))
                {
        ...
        public void Main(string argument, UpdateType updateSource)
        {
            ...
            tick++;
            Echo($"ExcavOS is running {spinner.Substring(tick % spinner.Length, 1)}");

            foreach (var cockpit in cockpits.Values)
            {
                if (!cockpit.WasUnderControl && cockpit.Cockpit.IsUnderControl && !excavOSContext.IsLoadingScreenDisabled)
                {
                ...
                else if (cockpit.WasUnderControl && !cockpit.Cockpit.IsUnderControl && !excavOSContext.IsLoadingScreenDisabled)
                {
                    // user exited cockpit
                ...
                else if (cockpit.Cockpit.IsUnderControl)
                {
                    ...
                    cockpit.Screens.ForEach(screen =>
                    {
                        if (excavOSContext.IsLoadingScreenDisabled)
                        {
